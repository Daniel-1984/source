...
/*/{Protheus.doc} WSRESTFUL JDAPIVEN
API -CRUD-VENDEDORES
...
WSRESTFUL JDAPIVEN DESCRIPTION 'API -CRUD-VENDEDORES'
    //Atributos
    WSDATA id         AS STRING
    ...
    //Métodos
    WSMETHOD GET    ID     DESCRIPTION 'Retorna o registro pesquisado' WSSYNTAX '/JDAPIVEN/get_id?{id}'                       PATH 'get_id'        PRODUCES APPLICATION_JSON
    WSMETHOD GET    ALL    DESCRIPTION 'Retorna todos os registros'    WSSYNTAX '/JDAPIVEN/get_all?{updated_at, limit, page}' PATH 'get_all'       PRODUCES APPLICATION_JSON
    WSMETHOD POST   NEW    DESCRIPTION 'Inclusão de registro'          WSSYNTAX '/JDAPIVEN/new'                               PATH 'new'           PRODUCES APPLICATION_JSON
    WSMETHOD PUT    UPDATE DESCRIPTION 'Atualização de registro'       WSSYNTAX '/JDAPIVEN/update'                            PATH 'update'        PRODUCES APPLICATION_JSON
    WSMETHOD DELETE ERASE  DESCRIPTION 'Exclusão de registro'          WSSYNTAX '/JDAPIVEN/erase'                             PATH 'erase'         PRODUCES APPLICATION_JSON
END WSRESTFUL
...
WSMETHOD GET ID WSRECEIVE id WSSERVICE JDAPIVEN
    Local lRet       := .T.
    ...
    Else
        ...
        Else
            ...
            jResponse['bairro'] := (cAliasWS)->A3_BAIRRO 
...
WSMETHOD GET ALL WSRECEIVE updated_at, limit, page WSSERVICE JDAPIVEN
    Local lRet       := .T.
    Local jResponse  := JsonObject():New()
    Local cQueryTab  := ''
    Local nTamanho   := 10
    ...
    // então você pode substituir o campo 'A3_USERLGA' por S_T_A_M_P_, I_N_S_D_T_ ou outro campo de data da tabela
    If ! Empty(::updated_at)
        cQueryTab += "     AND ((CASE WHEN SUBSTRING(A3_USERLGA, 03, 1) != ' ' THEN " + CRLF
        cQueryTab += "        CONVERT(VARCHAR,DATEADD(DAY,((ASCII(SUBSTRING(A3_USERLGA,12,1)) - 50) * 100 + (ASCII(SUBSTRING(A3_USERLGA,16,1)) - 50)),'19960101'),112) " + CRLF
        cQueryTab += "        ELSE '' " + CRLF
    ...
    Else
        ...
        While ! QRY_TAB->(EoF())
            ...
            oRegistro['comis'] := (cAliasWS)->A3_COMIS 
            oRegistro['msblql'] := (cAliasWS)->A3_MSBLQL 
...
WSMETHOD POST NEW WSRECEIVE WSSERVICE JDAPIVEN
    Local lRet              := .T.
    Local aDados            := {}
    Local jJson             := Nil
    ...
    Else
		...
		aAdd(aDados, {'A3_FILIAL',   jJson:GetJsonObject('filial'),   Nil})
		aAdd(aDados, {'A3_COD',   jJson:GetJsonObject('cod'),   Nil})
		aAdd(aDados, {'A3_NOME',   jJson:GetJsonObject('nome'),   Nil})
		aAdd(aDados, {'A3_NREDUZ',   jJson:GetJsonObject('nreduz'),   Nil})
		aAdd(aDados, {'A3_BAIRRO',   jJson:GetJsonObject('bairro'),   Nil})
		aAdd(aDados, {'A3_CEP',   jJson:GetJsonObject('cep'),   Nil})
		aAdd(aDados, {'A3_EMAIL',   jJson:GetJsonObject('email'),   Nil})
		aAdd(aDados, {'A3_COMIS',   jJson:GetJsonObject('comis'),   Nil})
		aAdd(aDados, {'A3_MSBLQL',   jJson:GetJsonObject('msblql'),   Nil})
		aAdd(aDados, {'A3_END',   jJson:GetJsonObject('end'),   Nil})
		...
		If lMsErroAuto
			...
			cArqLog := 'JDAPIVEN_New_' + dToS(Date()) + '_' + StrTran(Time(), ':', '-') + '.log'
...
WSMETHOD PUT UPDATE WSRECEIVE id WSSERVICE JDAPIVEN
    Local lRet              := .T.
    Local aDados            := {}
    Local jJson             := Nil
    Local cJson             := Self:GetContent()
    ...
    Else
        ...
        Else
            ...
            Else
		         ...
		         MsExecAuto({|x, y| MATA040(x, y)}, aDados, 4)
...
WSMETHOD DELETE ERASE WSRECEIVE id WSSERVICE JDAPIVEN
    Local lRet              := .T.
    Local aDados            := {}
    Local jJson             := Nil
    Local cJson             := Self:GetContent()
    ...
    Else
        ...
        Else
            ...
            Else
		         ...
		         MsExecAuto({|x, y| MATA040(x, y)}, aDados, 5)
